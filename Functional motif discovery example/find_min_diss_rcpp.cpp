...
// [[Rcpp::export]]
NumericVector find_diss_nicco(const List &y,const List &v,  
...
NumericVector find_diss_efficient(const List &y,const List &v,
                        ...
                        const Function & diss_d0_d1_L2)
{
  LogicalVector v_dom = as<LogicalVector>(domain(v,use0)); //conversione necessaria
  Rcpp::List v_new = select_domain(v, v_dom, use0, use1);
  int v_len = v_dom.size();
  int y_len = as<mat>(y[0]).n_rows; //conversione necessaria 
  IntegerVector s_rep = myseq(1 - (v_len - c_k), y_len - v_len + 1 + (v_len - c_k));
  
  List y_rep(s_rep.size());
  
  mat temp_y0, temp_y1; //@TODO : inizializzare con le size
  if (use0) {
    temp_y0 = as<mat>(y[0]); 
  }
  if (use1) {
 ...
 IntegerVector index(v_len);
 int index_j = 0;
 mat new_y0(index.size(), d);
 mat new_y1(index.size(), d);
 //bool not_stop_for0 = true; 
 //bool not_stop_for1 = true; 
 for (int i = 0; i < s_rep.size(); i++) {
   index = s_rep[i] - 1 + myseq(1,v_len);
   List y_rep_i = List::create(Named("y0") = R_NilValue, Named("y1") = R_NilValue); 
   if (use0) {
     new_y0.fill(datum::nan);
     for (unsigned int j = 0; j < index.size() /*&& not_stop_for0*/; j++) { // @TODO: accorpare in un solo ciclo su index
       index_j = index[j]; 
       if ((index_j > 0) && (index_j <= y_len)) {
         new_y0.row(j) = temp_y0.row(index_j - 1); 
       }
       /*if (index_j > y_len){
        not_stop_for0 = false;
       }*/
   ...
   if (use1) {
     new_y1.fill(datum::nan);
     for (int j = 0; j < index.size() /*&& not_stop_for1*/; j++) {
       index_j = index[j];
       if ((index_j > 0) && (index_j <= y_len)){
         new_y1.row(j) = temp_y1.row(index_j - 1); 
       }
       /*if (index_j > y_len){
         not_stop_for1 = false;
       }*/
