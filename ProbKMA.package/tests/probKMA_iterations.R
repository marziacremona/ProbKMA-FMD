...
load(paste0('../Data/len200_sd0.1.RData')) 
...
K = 2
c = 61
my_output = ProbKMA.package::probKMA(Y0,Y1,standardize=FALSE,K=K,c=c,c_max=c_max,P0=NULL,S0=NULL,
                 diss=diss,alpha=alpha,w=1,m=2,
                 iter_max=100,stop_criterion='max',quantile=NULL,tol=1e-8,
...
result_b <- microbenchmark(prof =probKMA(Y0,Y1,standardize=FALSE,K=K,c=c,c_max=c_max,P0=NULL,S0=NULL,
                                                    diss=diss,alpha=alpha,w=1,m=2,
                                                    iter_max=20,stop_criterion='max',quantile=NULL,tol=1e-8,
                                                    iter4elong=2,tol4elong=1e-3,max_elong=0.5,trials_elong=10,deltaJk_elong=0.05,max_gap=max_gap,
                                                    iter4clean=50,tol4clean=1e-4,quantile4clean=1/K,
                                                    return_options=TRUE,return_init=TRUE,worker_number=NULL),mio <- ProbKMA.package::probKMA(Y0,Y1,standardize=FALSE,K=K,c=c,c_max=c_max,P0=NULL,S0=NULL,
                                                                                                                                          diss=diss,alpha=alpha,w=1,m=2,
                                                                                                                                          iter_max=20,stop_criterion='max',quantile=NULL,tol=1e-8,
                                                                                                                                          iter4elong=2,tol4elong=1e-3,max_elong=0.5,trials_elong=10,deltaJk_elong=0.05,max_gap=max_gap,
                                                                                                                                          iter4clean=50,tol4clean=1e-4,quantile4clean=1/K,
                                                                                                                                          return_options=TRUE,return_init=TRUE,worker_number=NULL),times = 10)
print(result_b)
