...
load(paste0('cluster_candidate_motifs_results.RData'))

prof <- motifs_search(cluster_candidate_motifs_results,
              R_all=cluster_candidate_motifs_results$R_all,use_real_occurrences = TRUE)

motifs_search_custom <- function(cluster_candidate_motifs_results,
                          R_all=cluster_candidate_motifs_results$R_all,D_clean,R_m=NULL,
                          use_real_occurrences=TRUE,length_diff=Inf,worker_number=NULL){
  ...
  result <- motifs_search_cpp(Y,V,cluster_candidate_motifs_results$V0_clean,
                              cluster_candidate_motifs_results$V1_clean,
                              V_dom,V_length,cluster_candidate_motifs_results$P_clean,
                              cluster_candidate_motifs_results$D_clean,V_hclust,
                              cluster_candidate_motifs_results$alpha,use0,use1,w,
                              cluster_candidate_motifs_results$c,
                              cluster_candidate_motifs_results$max_gap,
                              d,N,K,R_all,R_m,use_real_occurrences,length_diff,
                              .diss_d0_d1_L2,
                              .domain,
                              .select_domain)
  #### outuput
  if(cluster_candidate_motifs_results$diss=='d0_L2'){
    V0=lapply(result$V,function(v) v$v0)
    V1=cluster_candidate_motifs_results$V1_clean[result$index_final]
  }else if(cluster_candidate_motifs_results$diss=='d1_L2'){
    V0=cluster_candidate_motifs_results$V0_clean[result$index_final]
    V1=lapply(result$V,function(v) v$v1)
  }else{
    V0=lapply(result$V,function(v) v$v0)
    V1=lapply(result$V,function(v) v$v1)
  }
  return(list(V0=V0,V1=V1,
              V_length=result$V_length,V_occurrences=result$V_occurrences,V_frequencies=result$V_frequencies,
              V_mean_diss=result$V_mean_diss,Y0=cluster_candidate_motifs_results$Y0,
              Y1=cluster_candidate_motifs_results$Y1,R_motifs=result$R_motifs))
}

mio <- motifs_search_custom(cluster_candidate_motifs_results,
