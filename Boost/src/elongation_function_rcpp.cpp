                     ...
                     const List& Y, int m, double deltaJk_elong) // ok 
{
  ...
  for (uword i=0; i < len_elong_k_zero_size;++i) {
    ...
    s_k_elong_left_right_temp.col(i) = s_k-len_elong_k_zero_i;
  }
  ...
  std::vector<LogicalVector> v_dom_elong_left_right; // we should use uvec for consistency?!
  ...
  if (!v_dom_elong_left_right.empty()) 
    v_dom_elong_left_right.pop_back();
  ...
  //Rcout << v_elong_left_right[0].size() << std::end;
  Rcout << "hello1" <<std::endl;
  ...
  std::for_each(not_NA_index.begin(),not_NA_index.end(),
                [&new_v_dom,&new_v,
                 ...
                 &s_k_elong_left_right](int j){new_v.push_back(as<List>(v_elong_left_right[j]));
                                //Rcout << v_elong_left_right[j].size() << std::endl;
  ...
  Rcout << "hello4" <<std::endl;
  // compute performance index before elongation
  double Jk_before = as<double>(compute_Jk(v_new_k, s_k, p_k, Y, alpha, w, m, use0=use0, use1=use1));
  Rcout << "hello5" <<std::endl;
  ...
  for (uword i = 0; i < v_elong_left_right_size; i++) {
    ...
    Jk_after(i) = as<double>(compute_Jk(v_i, s_i, c_i, p_k, Y, alpha, w, m, keep_k, use0, use1));
  }
  Rcout << "hello7" <<std::endl;
